...
// DosHeader describes dos header
...
// NewDosHeader creates a new DosHeader
...
// String retuns string representations
func (hdr *DosHeader) String() string {
	return sectionString(hdr.FileOffset, "IMAGE_DOS_HEADER", hdr.Data) + flagString(hdr.Flags)
}

// DosHeaderD describes dos header data
...
// NewNTHeader creates a new NTHeader
...
// NTHeaderD describes nt header data
type NTHeaderD struct {
	Signature uint32
}

// String returns string representation
func (hdr *NTHeader) String() string {
	return sectionString(hdr.FileOffset, "IMAGE_NT_HEADER", hdr.Data) + flagString(hdr.Flags)
}

// FileHeader describes file header
...
func NewFileHeader(fileOffset uint32) *FileHeader {
	header := &FileHeader{}
	header.Flags = make(map[string]bool)
	header.Size = uint32(binary.Size(header.Data))
	header.FileOffset = fileOffset
...
func (hdr *FileHeader) String() string {
	return sectionString(hdr.FileOffset, "IMAGE_FILE_HEADER", hdr.Data) + flagString(hdr.Flags)
}

// FileHeaderD represents file header data
...
// OptionalHeader describes optional header
...
// NewOptionalHeader creates new OptionalHeader
func NewOptionalHeader(fileOffset uint32) *OptionalHeader {
